...
class ZFS(Plugin):
    ...
    metrics = [
        ...
        CommandMetric("pool_list", [Command(["zpool", "list", "-v"], "ZFS Pool(s)", serializable=False)]),
        CommandMetric("pool_status", [Command(["zpool", "status", "-v"], "ZFS Pool(s) Status", serializable=False)]),
        CommandMetric(
            "pool_status_guid",
            [Command(["zpool", "status", "-g"], "ZFS Pool(s) Status (with vdev GUIDs)", serializable=False)],
        ),
        CommandMetric(
            "pool_status_dedup",
            [Command(["zpool", "status", "-D"], "ZFS Pool(s) Status (dedup statistics)", serializable=False)],
        ),
