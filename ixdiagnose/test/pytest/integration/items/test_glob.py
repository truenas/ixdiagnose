...
def validate_report_data(source_path, data, report, errors=None):
    ...
    base_dict = {
        ...
        "item_report": {"traceback": None, "copied_items": []},
    }
    ...
    with glob_data() as data:
        ...
        assert (
            all(
                [
                    os.path.isfile(
                        os.path.isfile(os.path.join(os.path.join(destination_dir, TestGlob.name), skip_dir[1:]))
                    ),
    ...
    with glob_data() as data:
        ...
        assert (
            validate_report_data(
                ...
                glob_artifact.debug_report,
                errors={
                    file_name: f"'{file_name}' exceeds specified 10 size"
                    for file_name in [*data["dirs"], *data["files"]]
                },
            )
            is True
        )
