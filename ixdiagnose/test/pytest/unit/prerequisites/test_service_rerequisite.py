...
@pytest.mark.parametrize(
    "name,args,returncode,should_work",
    [
...
def test_service_prerequisite(mocker, name, args, returncode, should_work):
    mocker.patch(
        "ixdiagnose.plugins.prerequisites.service.run",
        return_value=CompletedProcess(args=args, returncode=returncode, stdout="", stderr=""),
    )
