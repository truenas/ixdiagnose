...
@pytest.mark.parametrize(
    "name,cmds,return_values,input_file,output_file,should_work",
    [
        (
            "cmd",
            [
                Command(
                    ["lsblk", "-o", "NAME,FSTYPE,LABEL,UUID,PARTUUID", "-l", "-e", "230"], "lsblk", serializable=False
                )
            ],
            [
                CompletedProcess(
                    args=("lsblk", "-o", "NAME,FSTYPE,LABEL,UUID,PARTUUID", "-l", "-e", "230"), returncode=0, stderr=""
                )
            ],
            "cmd_context1.txt",
