...
@pytest.mark.parametrize(
    "dataset_name,props_dict,args,return_code,stdout,stderr,should_work",
    [
        (
...
@pytest.mark.parametrize(
    "resource_type,args,returncode,file_name,stderr,return_zfs_getacl,should_work",
    [
        ("zpool", ("zpool", "get", "all"), 0, "input_resource_output1.txt", "", None, False),
        (
            "filesystem",
            ("zfs", "get", "all"),
            0,
...
def test_resource_output(mocker, resource_type, args, returncode, file_name, stderr, return_zfs_getacl, should_work):
    ...
    mocker.patch("ixdiagnose.plugins.zfs.run", return_value=CompletedProcess(args, returncode, input_content, stderr))
